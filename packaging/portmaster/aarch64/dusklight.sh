...
export AURORA_SDL2SHIM_STRICT_PRESENT_SYNC="${AURORA_SDL2SHIM_STRICT_PRESENT_SYNC:-1}"
...
log_platform_info() {
  ...
  echo "AURORA_SDL2SHIM_STRICT_PRESENT_SYNC=${AURORA_SDL2SHIM_STRICT_PRESENT_SYNC:-}"
  echo "--- devices ---"
...
resolve_graphics_mode() {
  case "$DUSKLIGHT_PM_GRAPHICS_MODE" in
    auto|dawn-sdl2shim|dawn-sdl2shim-owned)
      echo "dawn-sdl2shim-owned"
      ;;
    diag)
      echo "diag"
      ;;
    *)
      echo "Unsupported DUSKLIGHT_PM_GRAPHICS_MODE=$DUSKLIGHT_PM_GRAPHICS_MODE; using dawn-sdl2shim-owned" >&2
      echo "dawn-sdl2shim-owned"
      ;;
  esac
}
...
GPTOKEYB_PID=""
GPTOKEYB_LOG="$GAMEDIR/gptokeyb.log"
finish_portmaster() {
...
if [ -x "$controlfolder/gptokeyb2" ]; then
  : > "$GPTOKEYB_LOG"
  env LD_PRELOAD="$controlfolder/libinterpose.${DEVICE_ARCH}.so" "$controlfolder/gptokeyb2" "dusklight.${DEVICE_ARCH}" -c "$GAMEDIR/dusklight.gptk" -Z -H select >> "$GPTOKEYB_LOG" 2>&1 &
  GPTOKEYB_PID=$!
  echo "Started gptokeyb2 pid=$GPTOKEYB_PID for PortMaster quit combo"
elif [ -n "${GPTOKEYB:-}" ]; then
  : > "$GPTOKEYB_LOG"
  $GPTOKEYB "dusklight.${DEVICE_ARCH}" -c "$GAMEDIR/dusklight.gptk" >> "$GPTOKEYB_LOG" 2>&1 &
  GPTOKEYB_PID=$!
  echo "Started gptokeyb pid=$GPTOKEYB_PID for PortMaster quit combo"
elif [ -x "$controlfolder/gptokeyb" ]; then
  : > "$GPTOKEYB_LOG"
  "$controlfolder/gptokeyb" "${ESUDOKILL:--1}" "dusklight.${DEVICE_ARCH}" -c "$GAMEDIR/dusklight.gptk" >> "$GPTOKEYB_LOG" 2>&1 &
  GPTOKEYB_PID=$!
  echo "Started gptokeyb pid=$GPTOKEYB_PID for PortMaster quit combo"
else
...
DUSKLIGHT_ARGS=(
  ...
  --cvar video.decoupleSimFromRender=true
