...
if ! "$DOCKER_BIN" version >/dev/null 2>&1; then
    ...
    if [[ -x "$WINDOWS_DOCKER" ]]; then
        ...
        DOCKER_ROOT_DIR="$(wslpath -w "$ROOT_DIR")"
    else
        echo "Docker is unavailable. Enable Docker Desktop WSL integration or set DOCKER_BIN." >&2
        exit 1
    fi
fi

"$DOCKER_BIN" build -f "$ROOT_DIR/packaging/portmaster/cmake-aarch64-focal-sdl2shim.Dockerfile" -t "$IMAGE" "$ROOT_DIR"

BUILD_DIR="$ROOT_DIR/build/portmaster-aarch64-focal-sdl2shim"
if [[ "$INCREMENTAL" == "1" && -f "$BUILD_DIR/build.ninja" ]]; then
    "$DOCKER_BIN" run --rm -v "$DOCKER_ROOT_DIR:/work" "$IMAGE" bash -lc '
set -euo pipefail
...
'
    ...
    "$DOCKER_BIN" run --rm -v "$DOCKER_ROOT_DIR:/work" "$IMAGE"
fi
