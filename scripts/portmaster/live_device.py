...
def verify(args: argparse.Namespace, password: str | None) -> None:
    remote_cmd = (
        ...
        f"grep -nE 'DUSKLIGHT_PM_GRAPHICS_MODE|AURORA_SDL2SHIM_STRICT_PRESENT_SYNC|SDL_VIDEODRIVER' "
...
def tail(args: argparse.Namespace, password: str | None) -> None:
    pattern = args.pattern or "graphics mode|Graphics adapter|framebuffer size|FATAL|terminate|ERROR"
