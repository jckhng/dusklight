...
if [[ ${#LIB_DIRS[@]} -gt 0 ]]; then
    ...
    # The executable needs SDL's SONAME, not duplicate copies of every source alias.
    if [[ -f "$STAGE_DIR/dusklight/lib.aarch64/libSDL3.so.0" ]]; then
        find "$STAGE_DIR/dusklight/lib.aarch64" -maxdepth 1 -type f \
            -name 'libSDL3.so*' ! -name 'libSDL3.so.0' -delete
    fi
fi
